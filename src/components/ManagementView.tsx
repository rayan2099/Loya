...
export const ManagementView: React.FC = () => {
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      {/* Top Header */}
      <div className="surface hero-sheen p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-[#263241] flex items-center gap-2">
            ...
            <span>{lang === 'ar' ? 'الفريق والصلاحيات' : 'Team & permissions'}</span>
          </h2>
          <p className="text-xs text-[#71717A] mt-1">
            {lang === 'ar'
              ? 'أضف الكاشير، وافق على طلبات الانضمام، وراقب جاهزية المحفظة من مكان واحد.'
              : 'Invite cashiers, approve join requests, and monitor wallet readiness from one place.'}
          </p>
        </div>
        ...
        {activeTab === 'staff' && (
          <button
            ...
            className="w-full sm:w-auto px-4 py-2.5 rounded-lg bg-gradient-to-br from-[#12A594] to-[#0B8A7D] text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all shrink-0"
          >
      ...
      <div className="surface p-1.5 flex gap-1.5">
        <button
          ...
          className={`flex-1 py-2.5 px-4 font-bold text-xs rounded-lg transition-colors flex items-center justify-center gap-2 ${
            activeTab === 'staff'
              ? 'text-[#0D9488] bg-[#ECFDF9]'
              : 'text-[#71717A] hover:text-[#3F4A5A] hover:bg-[#F8FAFC]'
          }`}
        >
          <Users className="w-4 h-4" />
          <span>{lang === 'ar' ? `الفريق (${employees.length})` : `Team (${employees.length})`}</span>
        </button>
        ...
        >
          ...
          <span>{lang === 'ar' ? 'المزامنة والإعدادات' : 'Sync & settings'}</span>
        </button>
      </div>
      ...
      {activeTab === 'staff' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="surface p-5 space-y-5 overflow-hidden relative">
            <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_12%_15%,rgba(37,99,235,0.08),transparent_18rem),radial-gradient(circle_at_85%_10%,rgba(13,148,136,0.11),transparent_16rem)]" />
            <div className="relative">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-0.5 rounded-full bg-[#ECFDF9] text-[#0D9488] text-[10px] font-bold border border-[#CFF5EC]">
                    {lang === 'ar' ? 'دعوة وانضمام الكاشير' : 'Quick Staff Invite'}
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      joinCodeStatus === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                    }`}
                  >
                ...
                <h3 className="font-bold text-base sm:text-lg flex items-center gap-2 text-[#263241]">
                  <span>{lang === 'ar' ? 'دعوة موظف بدون ارتباك' : 'Invite staff without friction'}</span>
                </h3>
                <p className="text-xs text-[#71717A]">
                  {lang === 'ar' ? 'اختر الدور، أرسل الدعوة، وسيظهر طلب الانضمام هنا للموافقة.' : 'Choose a role, send the invite, and approve the join request here.'}
                </p>
              </div>

              {/* Role Presets & Single Invite Button */}
              <div className="flex flex-wrap items-center gap-2 bg-white/80 p-2 rounded-xl border border-[#EEF0F2] backdrop-blur-md w-full md:w-auto">
                <div className="flex items-center gap-1.5 px-2">
                  <span className="text-[11px] text-[#71717A]">{lang === 'ar' ? 'الدور:' : 'Role:'}</span>
                  <select
                    value={inviteRolePreset}
                    onChange={(e) => setInviteRolePreset(e.target.value as any)}
                    className="bg-[#FFF7ED] text-[#C2410C] font-bold text-xs px-2 py-1 rounded-lg border border-[#FED7AA] outline-none"
                  >
                ...
                <button
                  ...
                  className="px-3.5 py-2 rounded-lg bg-gradient-to-br from-[#12A594] to-[#0B8A7D] text-white transition-all disabled:opacity-40 flex items-center gap-1.5 text-xs font-bold shadow-sm active:scale-95 cursor-pointer"
                >
                  <MessageCircle className="w-4 h-4" />
                  <span>{lang === 'ar' ? 'دعوة عبر واتساب' : 'Invite via WhatsApp'}</span>
                </button>

                <button
                  onClick={handleCopyCode}
                  title={lang === 'ar' ? 'نسخ الكود والرابط يدوياً' : 'Manual Copy'}
                  className="px-2.5 py-2 rounded-lg bg-[#F4F5F7] hover:bg-[#ECEFF3] text-[#536273] text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-4 border-t border-[#EEF0F2] text-center relative">
              <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-3 rounded-lg bg-[#EFF6FF]">
                <Download className="w-4 h-4 text-[#2563EB] shrink-0" />
                <span className="text-[11px] font-bold text-[#315466]">{lang === 'ar' ? '1- تحميل التطبيق' : '1- Download App'}</span>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-3 rounded-lg bg-[#FFF7ED]">
                <Key className="w-4 h-4 text-[#EA580C] shrink-0" />
                <span className="text-[11px] font-bold text-[#9A3412]">{lang === 'ar' ? `2- إدخال الرمز (${joinCode})` : `2- Enter Code (${joinCode})`}</span>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 p-3 rounded-lg bg-[#ECFDF9]">
                <Camera className="w-4 h-4 text-[#0D9488] shrink-0" />
                <span className="text-[11px] font-bold text-[#0F766E]">{lang === 'ar' ? '3- بدء المسح' : '3- Start Scanning'}</span>
              </div>
            </div>
            </div>
          </div>
          ...
          {employees.filter((emp) => emp.status === 'معلق').length > 0 && (
            <div className="surface p-4 sm:p-5 bg-[#FFFBF0] border-[#FFE08A] space-y-3 animate-in fade-in">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
                  <h4 className="font-bold text-sm text-[#7C3F12] flex items-center gap-1.5">
                  ...
                  .map((emp) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-[#263241]">{emp.name}</span>
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-auto">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-[#F4F5F7] hover:bg-[#ECEFF3] text-[#536273] font-bold text-xs transition-colors cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="px-4 py-1.5 rounded-lg bg-gradient-to-br from-[#12A594] to-[#0B8A7D] text-white font-bold text-xs shadow-sm active:scale-95 transition-all flex items-center gap-1 cursor-pointer"
                        >
          ...
          <div className="surface p-5 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-[#E2E8F0]">
              <div className="flex items-center gap-2">
                <h4 className="font-bold text-sm text-[#263241]">
            ...
            <div className="space-y-3">
              {employees.map((emp) => (
                  <div
                    key={emp.id}
                    onClick={() => setSelectedEmployee(emp)}
                    className="p-4 rounded-xl bg-[#FAFBFC] border border-[#EEF0F2] flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-white hover:border-[#0D9488]/40 transition-all shadow-sm cursor-pointer group/emp"
                  >
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h5 className="font-bold text-sm text-[#263241] group-hover/emp:text-[#0D9488] transition-colors">{emp.name}</h5>
                          <span
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="surface hero-sheen p-5 space-y-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-[#EEF0F2] pb-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-[#ECFDF9] text-[#0D9488] flex items-center justify-center border border-[#CFF5EC] shrink-0">
                  <CloudLightning className="w-6 h-6 animate-pulse" />
                </div>
                <div>
                  <h3 className="font-bold text-base sm:text-lg leading-snug text-[#263241]">
              ...
              <span className="self-start sm:self-auto px-3 py-1.5 rounded-full bg-[#ECFDF9] text-[#0D9488] font-bold text-xs shadow-sm shrink-0 flex items-center gap-1 border border-[#CFF5EC]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 rounded-xl bg-white/70 backdrop-blur-md border border-white">
                <span className="text-[11px] text-[#71717A] font-semibold block">
                  {lang === 'ar' ? 'عملاء اختاروا Apple Wallet' : 'Apple Wallet Customers'}
                </span>
                <span className="text-xl sm:text-2xl font-bold text-[#263241] mt-1 block">
                  {customers.filter((c) => c.walletStatus === 'apple' || c.walletStatus === 'both').length}
                </span>
                <span className="text-[10px] text-[#0D9488] font-bold block mt-0.5">{lang === 'ar' ? 'مسجلون في النظام' : 'Tracked in Loya'}</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-white/70 backdrop-blur-md border border-white">
                ...
                <span className="text-xl sm:text-2xl font-bold text-[#263241] mt-1 block">0</span>
                <span className="text-[10px] text-[#0D9488] font-bold block mt-0.5">{lang === 'ar' ? 'الطابور فارغ' : 'All delivered'}</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-white/70 backdrop-blur-md border border-white">
                ...
                <span className="text-xl sm:text-2xl font-bold text-[#263241] font-mono mt-1 block">38ms</span>
                <span className="text-[10px] text-[#0D9488] font-bold block mt-0.5">{lang === 'ar' ? 'فائق السرعة' : 'Ultra-low delay'}</span>
              </div>
            </div>
          </div>
      ...
      {showAddEmp && (
        <div className="modal-backdrop">
          <div className="modal-panel w-full max-w-md p-6 space-y-4">
            <h3 className="font-bold text-base text-[#263241]">{lang === 'ar' ? 'إضافة موظف / كاشير جديد للفرع' : 'Add Branch Cashier'}</h3>
      ...
      {newlyCreatedEmp && (
        <div className="modal-backdrop">
          <div className="modal-panel w-full max-w-md p-6 sm:p-7 space-y-5 text-center max-h-[90vh] overflow-y-auto">
